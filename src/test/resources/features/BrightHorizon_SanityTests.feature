...
Feature: Sanity Tests
  ...
  @SanityTests @TC_002
  @severity=High
  Scenario: TC002_BrightHorizons_WebSite_Search_ChildCare_Locations
    Given Web Application launched - Application - "BrightHorizons"
  ...
  @TC_003 @FailureCase
  @severity=High
  Scenario: TC001_BrightHorizons_WebSite_CenterSearch_FailureCase
    Given Web Application launched - Application - "BrightHorizons"
    And click on - search - icon on top right of the screen
    Then validate that the popup text box for search is loaded on the screen
    And enter the text "Employee Education in 2018: Strategies to W" to the search box and click on the search button
    Then validate that the first search result is the exact match of the searched text in the above step
